...
export default function ChatPanel({ hasDocuments }) {
  ...
  return (
    <main className="flex-1 flex flex-col overflow-hidden bg-slate-50">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <span className="text-xs text-gray-600">
              RAG
              {!hasDocuments && (
                <span className="text-gray-400 ml-1">(sin docs)</span>
              )}
            </span>
          </label>
        </div>
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto chat-scroll px-4 py-5 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 gap-3 pb-16">
            ...
            <p className="text-sm font-medium text-gray-500">
              {hasDocuments
                ...
                : 'Subí un documento en el panel izquierdo y empezá a preguntar'}
            </p>
          </div>
        )}
      ...
      <div className="flex-shrink-0 bg-white border-t border-gray-200 px-4 py-3">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 text-sm border border-gray-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-400 placeholder-gray-400 resize-none disabled:bg-gray-50 max-h-32 overflow-y-auto"
  ...
  if (msg.role === 'error') {
    return (
      <div className="flex justify-start">
        <div className="max-w-[75%] bg-red-50 border border-red-200 text-red-600 text-sm rounded-2xl rounded-bl-sm px-4 py-2.5 leading-relaxed">
          {msg.content}
        </div>
      </div>
    )
  ...
  return (
    <div className="flex justify-start gap-2.5">
      ...
      <div className="max-w-[75%]">
        <div className="bg-white border border-gray-200 text-gray-800 text-sm rounded-2xl rounded-bl-sm px-4 py-2.5 leading-relaxed shadow-sm whitespace-pre-wrap">
          {msg.content}
        </div>
